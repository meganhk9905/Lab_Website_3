...
app.get('/home/pick_color', function(req, res) {
    ...
    .then(info => {
    	res.render('pages/home',{
				...
				color_msg: info[1] // Return the color message
			})
    })
    .catch(err => {
            console.log('error', err);
...
app.get('/team_stats', function(req, res) {
	var viewgames = "select * from football_games where game_date between '2019-01-01' and '2019-12-31';";
	var numwin = "select count(*) from football_games where game_date between '2019-01-01' and '2019-12-31' and home_score > visitor_score;";
	var numlose = "select count(*) from football_games where game_date between '2019-01-01' and '2019-12-31' and home_score < visitor_score;";
	db.task('get-everything', task => {
	return task.batch([
		task.any(viewgames),
		task.any(numwin),
	...
	res.render('pages/team_stats', {
		my_title: "Football Team Stats",
		data: info[0],
		numwin: info[1][0].count,
		numlose: info[2][0].count
		})
	}).catch(err => {
            console.log('error', err);
...
app.get('/player_info', function(req, res) {
	...
	res.render('pages/player_info', {
		my_title: 'Football Players Info',
		data: result[0],
		selectplayer: '',
		total_games: ''
		})
...
app.get('/player_info/select_player', function(req, res) {
	var id = req.query.player_id;
	var player_info = 'select id, name from football_players;';
	var player= "select * from football_players where id = '" + id + "';";
	var total_games = "select count(*) from football_games where '" + id + "' = any(players);";
	db.task('get-everything', task => {
	return task.batch([
		task.any(player_info),
		task.any(player),
	...
	res.render('pages/player_info', {
		my_title: "Football Players Info",
		data: result[0],
		selectplayer: result[1][0],
		total_games: result[2][0]
		})
	}).catch(err => {
		console.log('error', err);
		response.render('pages/player_info', {
